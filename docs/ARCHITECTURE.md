...
`search_docs` is the primary tool. It parses natural language to detect a target library — `"search nextjs for ISR"` targets only `docs_nextjs`; `"search docs for routing"` searches all collections and filters results to a relevance score above 0.75 to avoid hallucination. Library name aliases (`ts`, `next` etc.) are resolved via a `LIBRARY_ALIASES` dict in `docs_server.py`.
