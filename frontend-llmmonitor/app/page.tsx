...
function StepBadge({ type }: { type: string | null }) {
  ...
  return (
    <span className={`px-1.5 py-0.5 rounded text-xs font-mono border ${s.badge} shrink-0 w-20 text-center`}>
      {type ?? '—'}
    </span>
  );
}
...
function ContextBadges({ step }: { step: TaskStep }) {
  return (
    <div className="flex gap-1 shrink-0">
      {!!step.skeleton_injected && <span className="bg-purple-950 text-purple-300 border border-purple-800 px-1 py-0.5 rounded text-xs">skel</span>}
      {!!step.chunks_injected   && <span className="bg-green-950 text-green-300 border border-green-800 px-1 py-0.5 rounded text-xs">chunks</span>}
      {!!step.verify_injected   && <span className="bg-yellow-950 text-yellow-300 border border-yellow-800 px-1 py-0.5 rounded text-xs">verify</span>}
    </div>
  );
}
...
function ModelBadge({ model }: { model: string | null }) {
  ...
  return (
    <span className="bg-gray-900 text-gray-400 border border-gray-700 px-1.5 py-0.5 rounded text-xs font-mono shrink-0" title={model}>
      {label}
    </span>
  );
}
  ...
  return (
    <div className={`border-b last:border-b-0 ${s.row}`}>
      ...
      >
        ...
        <span className="text-xs text-gray-600 font-mono w-14 shrink-0 text-right">
          {step.latency_ms != null ? `${(step.latency_ms / 1000).toFixed(1)}s` : '—'}
        </span>
        <span className="text-xs text-gray-500 truncate flex-1">{preview || '—'}</span>
        <ContextBadges step={step} />
        {(step.prompt_tokens != null || step.completion_tokens != null) && (
          <span className="text-xs text-gray-600 font-mono shrink-0 whitespace-nowrap">
            {step.prompt_tokens ?? '?'}↑ {step.completion_tokens ?? '?'}↓
          </span>
        )}
        <ModelBadge model={step.model} />
        <span className="text-xs text-gray-700 font-mono shrink-0">#{step.id}</span>
      </div>
          ...
          {!detail ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <pre className="bg-blue-950 border border-blue-900 rounded p-3 text-xs font-mono text-blue-100 whitespace-pre-wrap leading-relaxed overflow-y-auto max-h-[50vh]">
              ...
              <div>
                ...
                <pre className="bg-green-950 border border-green-900 rounded p-3 text-xs font-mono text-green-100 whitespace-pre-wrap leading-relaxed overflow-y-auto max-h-[50vh]">
  ...
  return (
    <div className="border border-gray-800 rounded mb-2 overflow-hidden">
      ...
      >
        ...
        <span className="text-gray-200 text-sm flex-1 truncate" title={task.user_task ?? undefined}>
          {task.user_task ?? <span className="text-gray-600 italic">no task text</span>}
        </span>

        {task.repo && (
          <span className="bg-blue-950 text-blue-300 border border-blue-800 px-2 py-0.5 rounded text-xs font-mono shrink-0">
            {task.repo}
          </span>
        )}

        <span className="text-xs text-gray-600 font-mono shrink-0">{task.step_count} steps</span>
        <span className="text-xs text-gray-600 font-mono shrink-0">{totalSec}</span>
    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-3 py-1 rounded text-xs bg-gray-900 text-gray-400 border border-gray-700 hover:bg-gray-800 transition-colors"
          >
      ...
      {showConfig && (
        <div className="mb-6 border border-gray-800 rounded bg-gray-950 p-4">
          {!config ? (
            <p className="text-xs text-gray-600">No config snapshot found — restart the proxy to record one.</p>
          ) : (
            <>
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Proxy Config</span>
                <span className="text-xs text-gray-600 font-mono">snapshot {new Date(config.timestamp).toLocaleString()}</span>
              </div>
              <div className="grid grid-cols-2 gap-x-8 gap-y-1.5">
                {Object.entries(config.config).map(([key, value]) => (
                  <div key={key} className="flex items-baseline gap-2">
                    <span className="text-xs text-gray-500 font-mono w-44 shrink-0">{key}</span>
                    <span className="text-xs text-gray-200 font-mono truncate" title={String(value)}>{String(value)}</span>
                  </div>
                ))}
              </div>
            </>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.entries(STEP_STYLE).map(([type, s]) => (
          <span key={type} className={`px-1.5 py-0.5 rounded text-xs font-mono border ${s.badge}`}>{type}</span>
        ))}
      </div>
